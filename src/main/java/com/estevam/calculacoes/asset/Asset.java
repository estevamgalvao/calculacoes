    ...
    private void recalculatePositionAndProfit() {
        ...
        for (Operation op : operations) {
            if (op.getType() == OperationType.BUY) {
                ...
                BigDecimal totalInvested = avgPrice.multiply(qty);
                totalInvested = totalInvested.add(op.getPrice().multiply(op.getQuantity()));
                qty = qty.add(op.getQuantity());
                avgPrice = qty.compareTo(BigDecimal.ZERO) > 0
                        ? totalInvested.divide(qty, 10, RoundingMode.HALF_UP)
                        : BigDecimal.ZERO;

            } else if (op.getType() == OperationType.SELL) {
                // Calculate profit based on average price BEFORE the sale
                if (qty.compareTo(op.getQuantity()) >= 0) {
                    BigDecimal profit = op.getPrice().subtract(avgPrice).multiply(op.getQuantity());
                    profitLoss = profitLoss.add(profit);
                    qty = qty.subtract(op.getQuantity());
                } else {
                    ...
                    BigDecimal profit = op.getPrice().subtract(avgPrice).multiply(qty);
                    profitLoss = profitLoss.add(profit);
                    qty = BigDecimal.ZERO;
        ...
        this.quantity = qty;
        this.totalValue = this.averagePrice.multiply(this.quantity);
